...
class GTJA_191(object):
    ...
    def alpha_004(self):
        ...
        #factor1=pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns)#[condition2]
        #factor2=-pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns)#[condition3]

        #part0=pd.rolling_sum(self.close,8)/8
        #part1=factor2[condition1].fillna(0)
        #part2=(factor1[~condition1][condition2]).fillna(0)
        #part3=(factor1[~condition1][~condition2][condition3]).fillna(0)
        #part4=(factor2[~condition1][~condition2][~condition3]).fillna(0)
    ...
    def alpha_006(self):
        ...
        #factor1=pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns) 
        #factor2=pd.DataFrame(np.zeros(self.close.shape),index=self.close.index,columns=self.close.columns)
        #factor3=-pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns) 
        #part1=factor1[condition1].fillna(0)
        #part2=factor2[condition2].fillna(0)
        #part3=factor3[condition3].fillna(0)
    ...
    def alpha_048(self):
        ...
        #factor1=pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns)[condition1].fillna(0)
        #factor2=pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns)[condition2].fillna(0)
        #factor3=pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns)[condition3].fillna(0)

        #factor11=-pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns)[(~condition1)&(self.close!=self.close.shift())].fillna(0)
        #factor22=-pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns)[(~condition2)&(self.close.shift()!=self.close.shift(2))].fillna(0)
        #factor33=-pd.DataFrame(np.ones(self.close.shape),index=self.close.index,columns=self.close.columns)[(~condition3)&(self.close.shift(2)!=self.close.shift(3))].fillna(0)

        #summ=factor1+factor2+factor3+factor11+factor22+factor33  
...
class alpha191:
    ...
    def getalpha191(df,p):
            stock = GTJA_191(df, df_benchmark=df)
            df['alpha191-001'] = stock.alpha_001()
            df['alpha191-002'] = stock.alpha_002()
            df['alpha191-003'] = stock.alpha_003()
            df['alpha191-004'] = stock.alpha_004()
            df['alpha191-005'] = stock.alpha_005()
            df['alpha191-006'] = stock.alpha_006()
            df['alpha191-007'] = stock.alpha_007()
            df['alpha191-008'] = stock.alpha_008()
            df['alpha191-009'] = stock.alpha_009()
            df['alpha191-010'] = stock.alpha_010()
            df['alpha191-011'] = stock.alpha_011()
            df['alpha191-012'] = stock.alpha_012()
            df['alpha191-013'] = stock.alpha_013()
            df['alpha191-014'] = stock.alpha_014()
            df['alpha191-015'] = stock.alpha_015()
            df['alpha191-016'] = stock.alpha_016()
            df['alpha191-017'] = stock.alpha_017()
            df['alpha191-018'] = stock.alpha_018()
            df['alpha191-019'] = stock.alpha_019()
            df['alpha191-020'] = stock.alpha_020()
            df['alpha191-021'] = stock.alpha_021()
            df['alpha191-022'] = stock.alpha_022()
            df['alpha191-023'] = stock.alpha_023()
            df['alpha191-024'] = stock.alpha_024()
            df['alpha191-025'] = stock.alpha_025()
            df['alpha191-026'] = stock.alpha_026()
            df['alpha191-027'] = stock.alpha_027()
            df['alpha191-028'] = stock.alpha_028()
            df['alpha191-029'] = stock.alpha_029()
            df['alpha191-031'] = stock.alpha_031()
            df['alpha191-032'] = stock.alpha_032()
            df['alpha191-033'] = stock.alpha_033()
            df['alpha191-034'] = stock.alpha_034()
            df['alpha191-035'] = stock.alpha_035()
            df['alpha191-036'] = stock.alpha_036()
            df['alpha191-037'] = stock.alpha_037()
            df['alpha191-038'] = stock.alpha_038()
            df['alpha191-039'] = stock.alpha_039()
            df['alpha191-040'] = stock.alpha_040()
            df['alpha191-041'] = stock.alpha_041()
            df['alpha191-042'] = stock.alpha_042()
            df['alpha191-043'] = stock.alpha_043()
            df['alpha191-044'] = stock.alpha_044()
            df['alpha191-045'] = stock.alpha_045()
            df['alpha191-046'] = stock.alpha_046()
            df['alpha191-047'] = stock.alpha_047()
            df['alpha191-048'] = stock.alpha_048()
            df['alpha191-049'] = stock.alpha_049()
            df['alpha191-050'] = stock.alpha_050()
            df['alpha191-051'] = stock.alpha_051()
            df['alpha191-052'] = stock.alpha_052()
            df['alpha191-053'] = stock.alpha_053()
            df['alpha191-054'] = stock.alpha_054()
            df['alpha191-055'] = stock.alpha_055()
            df['alpha191-056'] = stock.alpha_056()
            df['alpha191-057'] = stock.alpha_057()
            df['alpha191-058'] = stock.alpha_058()
            df['alpha191-059'] = stock.alpha_059()
            df['alpha191-060'] = stock.alpha_060()
            df['alpha191-061'] = stock.alpha_061()
            df['alpha191-062'] = stock.alpha_062()
            df['alpha191-063'] = stock.alpha_063()
            df['alpha191-064'] = stock.alpha_064()
            df['alpha191-065'] = stock.alpha_065()
            df['alpha191-066'] = stock.alpha_066()
            df['alpha191-067'] = stock.alpha_067()
            df['alpha191-068'] = stock.alpha_068()
            df['alpha191-069'] = stock.alpha_069()
            df['alpha191-070'] = stock.alpha_070()
            df['alpha191-071'] = stock.alpha_071()
            df['alpha191-072'] = stock.alpha_072()
            df['alpha191-073'] = stock.alpha_073()
            df['alpha191-074'] = stock.alpha_074()
            df['alpha191-075'] = stock.alpha_075()
            df['alpha191-076'] = stock.alpha_076()
            df['alpha191-077'] = stock.alpha_077()
            df['alpha191-078'] = stock.alpha_078()
            df['alpha191-079'] = stock.alpha_079()
            df['alpha191-080'] = stock.alpha_080()
            df['alpha191-081'] = stock.alpha_081()
            df['alpha191-082'] = stock.alpha_082()
            df['alpha191-083'] = stock.alpha_083()
            df['alpha191-084'] = stock.alpha_084()
            df['alpha191-085'] = stock.alpha_085()
            df['alpha191-086'] = stock.alpha_086()
            df['alpha191-087'] = stock.alpha_087()
            df['alpha191-088'] = stock.alpha_088()
            df['alpha191-089'] = stock.alpha_089()
            df['alpha191-090'] = stock.alpha_090()
            df['alpha191-091'] = stock.alpha_091()
            df['alpha191-092'] = stock.alpha_092()
            df['alpha191-093'] = stock.alpha_093()
            df['alpha191-094'] = stock.alpha_094()
            df['alpha191-095'] = stock.alpha_095()
            df['alpha191-096'] = stock.alpha_096()
            df['alpha191-097'] = stock.alpha_097()
            df['alpha191-098'] = stock.alpha_098()
            df['alpha191-099'] = stock.alpha_099()
            df['alpha191-100'] = stock.alpha_100()
            df['alpha191-101'] = stock.alpha_101()
            df['alpha191-102'] = stock.alpha_102()
            df['alpha191-103'] = stock.alpha_103()
            df['alpha191-104'] = stock.alpha_104()
            df['alpha191-105'] = stock.alpha_105()
            df['alpha191-106'] = stock.alpha_106()
            df['alpha191-107'] = stock.alpha_107()
            df['alpha191-108'] = stock.alpha_108()
            df['alpha191-109'] = stock.alpha_109()
            df['alpha191-110'] = stock.alpha_110()
            df['alpha191-111'] = stock.alpha_111()
            df['alpha191-112'] = stock.alpha_112()
            df['alpha191-113'] = stock.alpha_113()
            df['alpha191-114'] = stock.alpha_114()
            df['alpha191-115'] = stock.alpha_115()
            df['alpha191-116'] = stock.alpha_116()
            df['alpha191-117'] = stock.alpha_117()
            df['alpha191-118'] = stock.alpha_118()
            df['alpha191-119'] = stock.alpha_119()
            df['alpha191-120'] = stock.alpha_120()
            df['alpha191-121'] = stock.alpha_121()
            df['alpha191-122'] = stock.alpha_122()
            df['alpha191-123'] = stock.alpha_123()
            df['alpha191-124'] = stock.alpha_124()
            df['alpha191-125'] = stock.alpha_125()
            df['alpha191-126'] = stock.alpha_126()
            df['alpha191-128'] = stock.alpha_128()
            df['alpha191-129'] = stock.alpha_129()
            df['alpha191-130'] = stock.alpha_130()
            df['alpha191-131'] = stock.alpha_131()
            df['alpha191-132'] = stock.alpha_132()
            df['alpha191-133'] = stock.alpha_133()
            df['alpha191-134'] = stock.alpha_134()
            df['alpha191-135'] = stock.alpha_135()
            df['alpha191-136'] = stock.alpha_136()
            df['alpha191-137'] = stock.alpha_137()
            df['alpha191-138'] = stock.alpha_138()
            df['alpha191-139'] = stock.alpha_139()
            df['alpha191-140'] = stock.alpha_140()
            df['alpha191-141'] = stock.alpha_141()
            df['alpha191-142'] = stock.alpha_142()
            df['alpha191-144'] = stock.alpha_144()
            df['alpha191-145'] = stock.alpha_145()
            df['alpha191-147'] = stock.alpha_147()
            df['alpha191-148'] = stock.alpha_148()
            df['alpha191-150'] = stock.alpha_150()
            df['alpha191-151'] = stock.alpha_151()
            df['alpha191-152'] = stock.alpha_152()
            df['alpha191-153'] = stock.alpha_153()
            df['alpha191-154'] = stock.alpha_154()
            df['alpha191-155'] = stock.alpha_155()
            df['alpha191-156'] = stock.alpha_156()
            df['alpha191-157'] = stock.alpha_157()
            df['alpha191-158'] = stock.alpha_158()
            df['alpha191-159'] = stock.alpha_159()
            df['alpha191-160'] = stock.alpha_160()
            df['alpha191-161'] = stock.alpha_161()
            df['alpha191-162'] = stock.alpha_162()
            df['alpha191-163'] = stock.alpha_163()
            df['alpha191-164'] = stock.alpha_164()
            df['alpha191-167'] = stock.alpha_167()
            df['alpha191-168'] = stock.alpha_168()
            df['alpha191-169'] = stock.alpha_169()
            df['alpha191-170'] = stock.alpha_170()
            df['alpha191-171'] = stock.alpha_171()
            df['alpha191-172'] = stock.alpha_172()
            df['alpha191-173'] = stock.alpha_173()
            df['alpha191-174'] = stock.alpha_174()
            df['alpha191-175'] = stock.alpha_175()
            df['alpha191-176'] = stock.alpha_176()
            df['alpha191-177'] = stock.alpha_177()
            df['alpha191-178'] = stock.alpha_178()
            df['alpha191-179'] = stock.alpha_179()
            df['alpha191-180'] = stock.alpha_180()
            df['alpha191-182'] = stock.alpha_182()
            df['alpha191-184'] = stock.alpha_184()
            df['alpha191-185'] = stock.alpha_185()
            df['alpha191-186'] = stock.alpha_186()
            df['alpha191-187'] = stock.alpha_187()
            df['alpha191-188'] = stock.alpha_188()
            df['alpha191-189'] = stock.alpha_189()
            df['alpha191-191'] = stock.alpha_191() 
